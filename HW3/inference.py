...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        "*** YOUR CODE HERE ***"
        # print 'dist: ', self.getBeliefDistribution()
        ...
        for pos in self.legalPositions:
            ...
            if oldBelief[pos] > 0:
                newPosDist.divideAll(1.0/oldBelief[pos])
                newBelief += newPosDist
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        nPart = self.numParticles / len(self.legalPositions)
        self.particles = []
        for p in self.legalPositions:
            self.particles.extend([p] * nPart)
    ...
    def observe(self, observation, gameState):
        ...
        for p in self.legalPositions:
            ...
            weight[p] = self.particles.count(p) * emissionModel[distance]
        weight.normalize()
        weight.divideAll(1.0/self.numParticles)
        self.particles = []
        for p in self.legalPositions:
            self.particles.extend([p] * int(round(weight[p])))
    ...
    def elapseTime(self, gameState):
        ...
        for p in self.legalPositions:
            ...
            items = sorted(newPosDist.items())
            distribution = [i[1] for i in items]
            values = [i[0] for i in items]
            num = self.particles.count(p)
            tmp.extend(util.nSample(distribution, values, num))
        self.particles = tmp
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        combination = list(product(self.legalPositions, repeat=self.numGhosts))
        self.particles = []
        numSymbol = self.numParticles / len(combination)
        self.particles = combination * numSymbol
    ...
    def observeState(self, gameState):
        ...
        """
        for i in range(self.numGhosts):
            if emissionModels[i].totalCount() == 0: 
                self.initializeUniformly()
                return
            for p in self.particles:
                if noisyDistances[i] == None:
                    ...
                    weight[p] += emissionModels[i][distance]
        """
        for p in self.particles:
            prob = 1.0
            for i in range(self.numGhosts):
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            for index in range(self.numGhosts):
                newDist = getPositionDistributionForGhost(setGhostPositions(gameState, oldParticle[index]), i, self.ghostIndex[i])
                newParticle[index] = util.sampleFromCounter(newDist)
            "*** END YOUR CODE HERE ***"
            print newParticle
